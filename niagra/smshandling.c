...
#define URC_PROCESS_TASK_STACK_SIZE (1024*4) //(1024*2)
...
static UINT8 gUrcProcessTask1Stack[URC_PROCESS_TASK_STACK_SIZE] = {0xA5};
...
static sTaskRef gUrcProcessTask1 = NULL;
...
{  
    ...
    while(1)
    {
        ...
        {
		//	sAPI_Debug("sTask_UrcProcesser, gUrcMsgQueue:%p ,continue", gUrcMsgQueue);
		sAPI_UartPrintf("\n\r after recv1, gUrcMsgQueue:%p \n", gUrcMsgQueue);
		sAPI_Debug("\n\r after recv1, gUrcMsgQueue:%p \n", gUrcMsgQueue);
            continue;
        }
        ...
        {
            sAPI_Free(msg.arg3);
			sAPI_Debug("sTask_UrcProcesser, gUrcMsgQueue:%p ,continue free", gUrcMsgQueue);
            continue;
        }
			sAPI_UartPrintf("\n\r after recv2, gUrcMsgQueue:%p \n", gUrcMsgQueue);
		sAPI_Debug("\n\r after recv2, gUrcMsgQueue:%p \n", gUrcMsgQueue);
 ...
 void sAPP_UrcTask1(void)
{
    ...
    if (NULL != gUrcProcessTask1)
    {
		 sAPI_UartPrintf("%s  line %d\n", __func__,__LINE__);
        return;
    }
 sAPI_UartPrintf("%s  line %d\n", __func__,__LINE__);
    if (sAPI_TaskCreate(&gUrcProcessTask1,
        gUrcProcessTask1Stack,
