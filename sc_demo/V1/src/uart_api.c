...
void Uart3CBFuncEx(SC_Uart_Port_Number portNumber, int len, void *reserve)
{
    ...
    uartMsg.arg3 = uart3data;
    sendMsgToUartMsg(uartMsg);
#elif FEATURE_SIMCOM_PPPD
    int readLen = 0;
    memset(uart3data,0,len);
    readLen = sAPI_UartRead(portNumber, uart3data, len);
    SIM_MSG_T uartMsg = {0,0,0,NULL};
    uartMsg.msg_id = SRV_UART;
    uartMsg.arg1 = portNumber;
...
#endif
    //sAPI_Debug("%s, portNumber is %d, readlen[%d]:%s.", __func__, portNumber, readLen,uart3data);
    free((void *)uart3data);
    return;
}
...
{
#if (defined SIMCOM_A7680C_V5_01) || (defined SIMCOM_A7670C_V701) || (defined SIMCOM_A7670C_V702) || defined (SIMCOM_A7680C_V506) || defined (SIMCOM_A7680C_V801) || defined (SIMCOM_A7680C_V603) || defined (SIMCOM_A7605C1_V201) || defined (SIMCOM_A7673G_V201) || defined (SIM7600DM_V101)
    char *uart4Data = malloc(UART_RX_BUFFER_SIZE);

#ifndef SIM7600DM_V101
#ifdef SIMCOM_UI_DEMO_TO_UART1_PORT
    int readLen = 0;
    readLen = sAPI_UartRead(portNumber, (UINT8 *)uart4Data, UART_RX_BUFFER_SIZE);
    //sAPI_Debug("%s, portNumber is %d, readLen[%d]:%s.", __func__, portNumber, readLen,uart4Data);
    CacheDataToUIDemo(uart4Data, readLen);
#endif
#else
    //sAPI_Debug("%s, SIM7600DM %d, readLen[%d]:%s.", __func__, portNumber, readLen,uart4Data);
#endif
    free((void *)uart4Data);
#endif
    return;
...
void sDemo_UartInit(void)
{
...
#if ((defined (CUS_PUWELL) && (defined SIMCOM_A7690C_V201))) || ((!defined SIMCOM_A7690C_V201) && (!defined SIMCOM_A7680C_V801))
#ifndef FALCON_1803_PLATFORM
#ifdef PROJECT_TOPFLY
    ...
    SC_STATUS status;
    status = sAPI_MsgQCreate(&sc_uartmsgq, "sc_uartmsgq", sizeof(SIM_MSG_T), 64, SC_FIFO);
