...
typedef enum
{
...
#if (defined SC_DRIVER_GNSS_ASR) || (defined SC_DRIVER_GNSS_CC1161W) || (defined SC_DRIVER_GNSS_CC1177W) || (defined SC_DRIVER_GNSS_CC1167Q) || (defined SC_DRIVER_GNSS_AG3352) || (defined SC_DRIVER_GNSS_ZKW)
    ...
    SC_GNSS_DEMO_MODE_GALILEO                           = 8,
    SC_GNSS_DEMO_MODE_GPS_GALILEO                       = 9,
    SC_GNSS_DEMO_MODE_BDS_GALILEO                       = 10,
    SC_GNSS_DEMO_MODE_GPS_BDS_GALILEO                   = 11,
    SC_GNSS_DEMO_MODE_GLONASS_GALILEO                   = 12,
    SC_GNSS_DEMO_MODE_GPS_GLONASS_GALILEO               = 13,
    SC_GNSS_DEMO_MODE_BDS_GLONASS_GALILEO               = 14,
    SC_GNSS_DEMO_MODE_GPS_BDS_GLONASS_GALILEO           = 15,
    SC_GNSS_DEMO_MODE_GET                               = 16,
#else//UNIC6228
...
void GNSSDemo(void)
{
    ...
    char *mode_list[] =
    {
        ...
        "8. get mode",
#elif (defined SC_DRIVER_GNSS_CC1161W) || (defined SC_DRIVER_GNSS_CC1167Q)
        "1. set GPS mode",
        "2. set BDS mode",
        "3. set GPS+BDS mode",
        "4. set GLONASS mode",
        ...
        "7. set GPS+BDS+GLONASS mode",
        "8. set GALILEO mode",
        "9. set GPS+GALILEO mode",
        "10. set BDS+GALILEO mode",
        "11. set GPS+BDS+GALILEO mode",
        "12. set GLONASS+GALILEO mode",
        "13. set GPS+GLONASS+GALILEO mode",
        "14. set BDS+GLONASS+GALILEO mode",
        "15. set GPS+BDS+GLONASS+GALILEO mode",
        "16. get mode",
#elif defined SC_DRIVER_GNSS_CC1177W
        "1. set BDS mode",
        "2. get mode",
#elif defined SC_DRIVER_GNSS_AG3352
        "1. set GPS mode",
        "2. set GPS+BDS mode",
        "3. set GPS+GLONASS mode",
        "4. set GPS+GALILEO mode",
        "5. set GPS+BDS+GALILEO mode",
        "6. set GPS+BDS+GLONASS+GALILEO mode",
        "7. get mode",
#else//UNIC6228
    #ifdef FEATURE_SIMCOM_GPS_OVERSEAS
...
reMode:
                ...
                if (powerOn())
                {
                    switch (opt)
                    {
                        ...
                        case SC_GNSS_DEMO_MODE_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_GALILEO);
                            PrintfResp("\r\nset GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                        ...
                        case SC_GNSS_DEMO_MODE_GPS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_GPS_GALILEO);
                            PrintfResp("\r\nset GPS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_BDS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_BDS_GALILEO);
                            PrintfResp("\r\nset BDS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_GPS_BDS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_GPS_BDS_GALILEO);
                            PrintfResp("\r\nset GPS+BDS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_GLONASS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_GLONASS_GALILEO);
                            PrintfResp("\r\nset GLONASS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_GPS_GLONASS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_GPS_GLONASS_GALILEO);
                            PrintfResp("\r\nset GPS+gLONASS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_BDS_GLONASS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_BDS_GLONASS_GALILEO);
                            PrintfResp("\r\nset BDS+GLONASS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_GPS_BDS_GLONASS_GALILEO:
                        {
                            ret = sAPI_GnssModeSet(SC_GNSS_DEMO_MODE_GPS_BDS_GLONASS_GALILEO);
                            PrintfResp("\r\nset GPS+BDS+GLONASS+GALILEO mode!\r\n");
                            if (SC_GNSS_RETURN_CODE_ERROR == ret)
                            {
                                sAPI_Debug("%s: set mode error!", __func__);
                            }
                        ...
                        case SC_GNSS_DEMO_MODE_GET:
                        {
                            ...
                            switch (gnssMode)
                            {
                                ...
                                case SC_GNSS_MODE_GPS_BDS_GLONASS:
                                    PrintfResp("\r\nGPS+BDS+BLONASS mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_GALILEO:
                                    PrintfResp("\r\nGALILEO  mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_GPS_GALILEO:
                                    PrintfResp("\r\nGPS+GALILEO mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_BDS_GALILEO:
                                    PrintfResp("\r\nBDS+GALILEO mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_GPS_BDS_GALILEO:
                                    PrintfResp("\r\nGPS+BDS+GALILEO mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_GLONASS_GALILEO:
                                    PrintfResp("\r\nGLONASS+GALILEO mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_GPS_GLONASS_GALILEO:
                                    PrintfResp("\r\nGPS+GLONASS+GALILEO mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_BDS_GLONASS_GALILEO:
                                    PrintfResp("\r\nBDS+GLONASS+GALILEO mode!\r\n");
                                    break;
                                case SC_GNSS_MODE_GPS_BDS_GLONASS_GALILEO:
                                    PrintfResp("\r\nGPS+BDS+GLONASS+GALILEO mode!\r\n");
                                    break;
                                default:
                                    PrintfResp("\r\ncannot get mode!\r\n");
                                    break;
