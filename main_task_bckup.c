...
#define UART_RX_BUFFER_SIZE 200 // RX buffer can not more than 2048
...
static sTaskRef gUartRxProcessTask = NULL;
...
//static UINT8 gUartRxProcessTaskStack[5 * 1024] = {0xA5}; //[5 * 1024] = {0xA5};
...
char StrTokStr1[100][80]; //[260][80]; // 150,120 // dg_changed from 120 80
// char StrTokStr1[120][80]; // 150,120
char StrTokStr2[170][50]; // 160 , 100
...
//	strcpy((char *)pfile,(char*)"c:/vset.txt");
	...
	file_hdl = sAPI_fopen((UINT8*)pfile,"ab");
                if(file_hdl == NULL)
                {
                 //   PrintfResp("sAPI_fopen err :c:/rtcset%d.txt",f_pumpno);  
  ...
  ret = sAPI_fclose(file_hdl);
                 if(ret != 0)
                {
                 //   PrintfResp("c:/rtcset%d.txt sAPI_fclose err",f_pumpno);
...
file_hdl = sAPI_fopen((UINT8*)pfile,"ab");
                if(file_hdl == NULL)
                {
                //    PrintfResp("sAPI_fopen err :c:/ctset.txt",f_pumpno);  
	...
	//	g_no_of_pumps=1;
		...
		{
			sprintf(buf,"c:/ctset.txt FileRead()=%d: readedlen=%d, strBuf=%s\r\n",ret, readedlen, strBuf);
		PrintfResp(buf); 
			Pch = strtok((char *)strBuf, (char *)"," );
			StrTokStrVer = 0;
			while( Pch != NULL )
  ...
  ret = sAPI_fclose(file_hdl);
                 if(ret != 0)
                {
                  //  PrintfResp("c:/ctset.txt sAPI_fclose err",f_pumpno);
...
void WritectsetFile()
{
	...
	strcpy((char *)pfile,(char*)filename);
	 ...
	 file_hdl = sAPI_fopen((UINT8*)pfile, "wb");
                if(file_hdl == NULL)
                {
                    PrintfResp("sAPI_fopen err WritectsetFile");
...
void writeonoffbuff(void)
{
    ...
    UINT32 writeedlen;
	int Tp=0;
	PrintfResp("In OnOff Write\n");
	strcpy((char *)pfile,(char*)"onoffbuf.txt");
 	file_hdl = sAPI_fopen((UINT8*)pfile , "wb" );
	  if(file_hdl == NULL)
                {
                    PrintfResp("sAPI_FsFileOpen err");
...
//	char TempBuf1[500] = "";
	...
	memset(&TCPWifigprsstrBUFF, 0, 650);
	memset(&SmsStrNumber, 0, 8500);
	// memset(&TCPwifiStrNumber,0,150000);
	// memset(&Triprec,0,4004);
	memset(&StrTokStr1, 0, 4004); // 4004
	memset(&StrTokStr2, 0, 4004);
	...
	//	WriteonofFile();
		if (status == SC_SUCCESS)
		{
			...
			{
				sAPI_UartPrintf("DBG: cpbrsearchend == 0");
				ReadPhoneNumber();
				ph_numcheck();
				cpbrsearchend = 1;
			...
			if (datetime.tm_sec != No_comm_prev_secs)
			{
				No_comm_secs++;
...
s_nTimerSettings.m_CycLicOfSec[l_pumpno_tx] */
												...
												//case STATE_MOTOR1ON_SMS:
													//{
														...
														else if(nSTATE_MOTOR1_ON_SMS==STATE_MOTOR1_ON_RTC1)
														{
																...
																PrintfResp(Buffer4);//subash commented
																if(WorkingOn3Phas == 1)
																{
																	if(Appmodeon==1)
	...
	//		else if (((Nooftcpprocessed < Nooftcprecvd) || (Nooftcpprocessed1 < Nooftcprecvd1) || livedataflag == 1 || livedataflag1 == 1) && s_nMSettings.m_Enter > 70)
			else if(((Nooftcpprocessed < Nooftcprecvd)||(Nooftcpprocessed1 < Nooftcprecvd1)|| livedataflag==1||livedataflag1==1)&& s_nTimerSettings.m_Enter>=71)  //dg_nsdk
			{
...

				{
					...
					if (tcpdcounter++ > 5)
					{
						...
						/*	revalue = zone[nVaTr.Currentvalve].stoptime;
							...
							Motoronflag=0;
							sprintf(getbuff,"{\r\n\"cC\":\"%s\",\r\n\"cM\":\"%d,%03.0f,%03.0f,%03.0f,%03.0f,%03.0f,%03.0f,",IMEI,Motoronflag,nCurretnCond.RVoltage,nCurretnCond.YVoltage,nCurretnCond.BVoltage,nCurretnCond.RYVoltage,nCurretnCond.YBVoltage,nCurretnCond.BRVoltage);//,DIS_BUF,VAL_BUF,datetime.tm_mday,datetime.tm_mon,datetime.tm_year,datetime.tm_hour,datetime.tm_min,datetime.tm_sec); //dg_bckup
							...
							sprintf(getbuff,"%s%d,%03d,",getbuff,nVaTr.programselection,nDripSettings.stp);
							 ...
							 sprintf(getbuff,"%s%02ld:%02ld:%02ld,%02ld:%02ld:%02ld,%d,%d,100,100,%d,%d,%d,%d,000.0,%d,000.0,%d,%d,123456789,49.9,000,000,100,100,100,100,100,100,%.02fF-%02d,000.0,%d,%d,%d,%d,",getbuff,reTpHr,reTpMin,reTpSec,TpHr,TpMin,TpSec,runvflow,value,nVaTr.fertv1Smsonof,nVaTr.fertv2Smsonof,nVaTr.fertv3Smsonof,nVaTr.fertv4Smsonof,nVaTr.Currentflow, LastDayRunflow,Runflow,leveltank,levelpercent,CSQ,PrvLocanMotorStatus,PrvMSMotorStatus,nTimerSettings.ManualOnOff);
							...
							if(BATPER>=100) 
							BATPER=100;
							sAPI_UartPrintf("Battery Percentage:%d,%.0f\n\r",avg_adc,BATPER);
							...
							value = value%60;
							reTpSec = value;
							sprintf(getbuff,"%s%02ld:%02ld,%02ld:%02ld,%.0f,%.0f,",getbuff,TpHr,TpMin,reTpHr,reTpMin,(float)(CSQ*3.2258),BATPER);      //LIVE  
					...
					//		sprintf(getbuff,"{\r\n\"2400\": [\r\n{\r\n\"2407\": [ \r\n{\r\n\"ST\":1,\r\n\"RN\":12, \r\n\"WM\":1,\r\n\"PR\":12.5,\r\n\"LV\":100,\r\n\"FT\":\"1:1:1:1\"\r\n\"OD\":\"11:22:33\",\r\n\"ODC\":\"44:55:66\",\r\n\"ODL\":\"77:88:99\"\r\n},\r\n{\r\n\"ST\":1,\r\n\"RN\":12, \r\n\"WM\":1,\r\n\"PR\":12.5,\r\n\"LV\":100,\r\n\"FT\":\"1:1:1:1\"\r\n\"OD\":\"11:22:33\",\r\n\"ODC\":\"44:55:66\",\r\n\"ODL\":\"77:88:99\"\r\n},\r\n{\r\n\"ST\":1,\r\n\"RN\":12, \r\n\"WM\":1,\r\n\"PR\":12.5,\r\n\"LV\":100,\r\n\"FT\":\"1:1:1:1\"\r\n\"OD\":\"11:22:33\",\r\n\"ODC\":\"44:55:66\",\r\n\"ODL\":\"77:88:99\"\r\n},\r\n{\r\n\"V\":\"230,230,230\",\r\n\"C\":\"1:12.5,2:13.5,3:12.5\",\r\n\"cC\":\"864180055212232\"}\r\n]\r\n}\r\n]\r\n}");
						...
						/* 			sprintf(getbuff1,"{\r\n\"LD01\": [ \r\n{\r\n\"ST\":%01d,\r\n\"RN\":%01d, \r\n\"WM\":%01d,\r\n\"PR\":%03.01f,\r\n\"LV\":%d,\r\n\"FT\":\"%01d:%01d:%01d:%01d\",\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\"\r\n},",Motoronflag[0],Motorreasonflag[0],flowisthere[0],Act_pressure[0],Act_level[0],actual_float_stat[0][0],actual_float_stat[0][1],actual_float_stat[0][2],actual_float_stat[0][3],act_POnMin[0],act_POnSec[0],act_del_comp_min[0],act_del_comp_sec[0],act_rem_delmin[0],act_rem_delsec[0]);
									sprintf(buf,"\n\r getbuff1 %s",getbuff1);
									PrintfResp(buf);
									size=strlen(getbuff1);
									sprintf(buf,"\n\r size %d",size);
									PrintfResp(buf);
									sprintf(getbuff2,"\r\n{\r\n\"ST\":%01d,\r\n\"RN\":%01d, \r\n\"WM\":%01d,\r\n\"PR\":%03.01f,\r\n\"LV\":%d,\r\n\"FT\":\"%01d:%01d:%01d:%01d\",\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\"\r\n},\r\n",Motoronflag[1],Motorreasonflag[1],flowisthere[1],Act_pressure[1],Act_level[1],actual_float_stat[1][0],actual_float_stat[1][1],actual_float_stat[1][2],actual_float_stat[1][3],act_POnMin[1],act_POnSec[1],act_del_comp_min[1],act_del_comp_sec[1],act_rem_delmin[1],act_rem_delsec[1]);
									sprintf(buf,"\n\r getbuff2 %s",getbuff2);
									PrintfResp(buf);
									size=strlen(getbuff2);
									sprintf(buf,"\n\r size %d",size);
									PrintfResp(buf);
									sprintf(getbuff3,"{\r\n\"ST\":%01d,\r\n\"RN\":%01d, \r\n\"WM\":%01d,\r\n\"PR\":%03.01f,\r\n\"LV\":%d,\r\n\"FT\":\"%01d:%01d:%01d:%01d\",\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\"\r\n},",Motoronflag[2],Motorreasonflag[2],flowisthere[2],Act_pressure[2],Act_level[2],actual_float_stat[2][0],actual_float_stat[2][1],actual_float_stat[2][2],actual_float_stat[2][3],act_POnMin[2],act_POnSec[2],act_del_comp_min[2],act_del_comp_sec[2],act_rem_delmin[2],act_rem_delsec[2]);
									...
									sprintf(getbuff4,"\r\n{\r\n\"V\":\"%03d,%03d,%03d\",\r\n\"C\":\"1:%03.1f,2:%03.01f,3:%03.01f\",\r\n\"cC\":\"%s\"\r\n}\r\n]\r\n}",RVoltage1,YVoltage1,BVoltage1,nCurretnCond.Rcurrent,nCurretnCond.Ycurrent,nCurretnCond.Bcurrent,IMEI);
							...
							PrintfResp(buf); */	   
										...
										/* sprintf(getbuff1,"{\r\n\"cC\":\"%s\",\r\n\"cM\": [ \r\n{\r\n\"ST\":%01d,\r\n\"RN\":%01d, \r\n\"WM\":%01d,\r\n\"PR\":%03.01f,\r\n\"LV\":%d,\r\n\"FT\":\"%01d:%01d:%01d:%01d\",\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\"\r\n},",IMEI,Motoronflag[0],Motorreasonflag[0],flowisthere[0],Act_pressure[0],Act_level[0],actual_float_stat[0][0],actual_float_stat[0][1],actual_float_stat[0][2],actual_float_stat[0][3],act_POnMin[0],act_POnSec[0],act_del_comp_min[0],act_del_comp_sec[0],act_rem_delmin[0],act_rem_delsec[0]);
									sprintf(buf,"\n\r getbuff1 %s",getbuff1);
									PrintfResp(buf);
									size=strlen(getbuff1);
									sprintf(buf,"\n\r size %d",size);
									...
									sprintf(getbuff4,"\r\n{\r\n\"V\":\"%03d,%03d,%03d\",\r\n\"C\":\"1:%03.1f,2:%03.01f,3:%03.01f\",\r\n}\r\n],\r\n\"cD\":\"%d/%d/%02d\",\r\n\"cT\":\"%d:%d:%d\",\r\n\"mC\":\"LD01\"\r\n}",RVoltage1,YVoltage1,BVoltage1,nCurretnCond.Rcurrent,nCurretnCond.Ycurrent,nCurretnCond.Bcurrent,datetime.tm_year,datetime.tm_mon,datetime.tm_mday,datetime.tm_hour,datetime.tm_min,datetime.tm_sec);
					...
					//	sprintf(getbuff1,"{\r\n\"cC\":\"%s\",\r\n\"cM\": [ \r\n{\r\n\"ST\":%01d,\r\n\"RN\":%01d,\r\n\"AT\":%03.01f,\r\n\"SE\":%03.01f,\r\n\"PH\":%03.01f, \r\n\"WM\":%01d,\r\n\"CF\":%03.01f,\r\n\"PR\":%03.01f,\r\n\"LV\":%d,\r\n\"FT\":\"%01d:%01d:%01d:%01d\",\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\"\r\n},",IMEI,Motoronflag[0],Motorreasonflag[0],actual_value,set_value,phase_number,flowisthere[0],cumulative_flow,Act_pressure[0],Act_level[0],actual_float_stat[0][0],actual_float_stat[0][1],actual_float_stat[0][2],actual_float_stat[0][3],act_POnMin[0],act_POnSec[0],act_del_comp_min[0],act_del_comp_sec[0],act_rem_delmin[0],act_rem_delsec[0]);
					//			sprintf(getbuff1,"{\r\n\"cC\":\"%s\",\r\n\"cM\": [ \r\n{\r\n\"ST\":%01d,\r\n\"RN\":%01d,\r\n\"AT\":%03.01f,\r\n\"SE\":%03.01f,\r\n\"PH\":%03d,%s,%s,%s,\r\n\"FT\":\"%01d:%01d:%01d:%01d\",\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\"\r\n},",IMEI,Motoronflag[0],Motorreasonflag[0],actual_value,set_value,phase_number,flowbuf,presbuf,levelbuf,actual_float_stat[0][0],actual_float_stat[0][1],actual_float_stat[0][2],actual_float_stat[0][3],act_POnMin[0],act_POnSec[0],act_del_comp_min[0],act_del_comp_sec[0],act_rem_delmin[0],act_rem_delsec[0]);
								//sprintf(getbuff1,"{\r\n\"cC\":\"%s\",\r\n\"cM\": [ \r\n{\r\n\"ST\":%01d,\r\n\"RN\":%01d,\r\n\"AT\":%03.01f,\r\n\"SE\":%03.01f,\r\n\"PH\":%01d,%s,%s,%s,%s,\r\n\"OD\":\"00:%02d:%02d\",\r\n\"ODC\":\"00:%02d:%02d\",\r\n\"ODL\":\"00:%02d:%02d\",\r\n\"CFDL\":\"%02d:%02d:%02d\",\r\n\"CNDL\":\"%02d:%02d:%02d\"\r\n\},",IMEI,Motoronflag[0],Motorreasonflag[0],actual_value,set_value,phase_number,flowbuf,presbuf,levelbuf,flotbuf,act_POnMin[0],act_POnSec[0],act_del_comp_min[0],act_del_comp_sec[0],act_rem_delmin[0],act_rem_delsec[0],act_rem_cyc_ofHr[0],act_rem_cyc_ofMin[0],act_rem_cyc_ofSec[0],act_rem_cyc_onHr[0],act_rem_cyc_onMin[0],act_rem_cyc_onSec[0]);// "MR\":\"%02d:%02d:%02d\"\r\n  //,act_rem_maxHr[0],act_rem_maxMin[0],act_rem_maxSec[0]
					...
					{								
						...
						//	if(BATPER>=100) 
							...
							sAPI_UartPrintf("Battery Percentage:%d,%.0f cont_ver:G%d.%d\n\r",avg_adc,BATPER,nMSettings.CONT_VER[0],nMSettings.CONT_VER[1]);
									memset(getbuff3,NULL,sizeof(getbuff3));
...
void sAPP_Timer2(void *argv)
{
	...
	while (1)
	{
		...
		if (status == SC_SUCCESS)
		{
			...
			sAPI_NetworkGetCgreg(&CGREG);
			// sAPI_SimPinGet(Cpin);
			if (IMEI_req_flag == 0 && s_nMSettings.m_Enter >= 70)
			{
				sAPI_Debug("status[%d] event[%d]", status, event);
				sprintf(Sim_Buf, "$MI\n\r");
				sAPI_UartWrite(SC_UART, (UINT8 *)Sim_Buf, strlen(Sim_Buf));
			...
			sprintf(buf, "\n\rCSQ:%d,%d,%d,%d", CSQ, Creg, CGREG, Cpin);
			sAPI_UartPrintf(buf);
	...
	if (1 == format) // TEXT mode
	{
		...
		//	else if (strstr(smsbuffer, "dt,") != 0)
			else if((strstr(smsbuffer,"dt,") != 0) || (strstr(smsbuffer,"DT,") != 0)) //dg_nsdk
			{
				...
				/* datetime.tm_year=timeval.tm_year;
				datetime.tm_mon=timeval.tm_mon;
				datetime.tm_mday=timeval.tm_mday;
				datetime.tm_hour=timeval.tm_hour;
				datetime.tm_min=timeval.tm_min;
				datetime.tm_sec=timeval.tm_sec; */
...
/*	status = sAPI_MsgQCreate(&uart2_msgq, "uart2_msgq", (sizeof(SIM_MSG_T)), 20, SC_FIFO);
	if (status != SC_SUCCESS)
	{
	...
	/**************************   UART task   *************************************/
	/******************************************************************************/
	// if (NULL != gUartRxProcessTask)
	// {
	// 	return;
	// }
	sAPI_UartPrintf("sAPI_UartTaskCreate start!\n");

	// if (sAPI_TaskCreate(&gUartRxProcessTask,
	// 					gUartRxProcessTaskStack,
	// 					7 * 1024,
	// 					100,
...
{	  
		...
		memcpy(uart_read_buf,uartData,35);
		
		sAPI_Free(uartData);		 
		 
       
	   if(nMSettings.ndebugonof==1)		
		 sAPI_UartPrintf("UART:%s",uart_read_buf);		   
		...
		//	}						
						...
						if(number_enu_data[0]>=1 && (prev_number_enu_data[0] != number_enu_data[0]))//subash doubte to modify
						{
							...
							else if(number_enu_data[0]==6)
							{
								...
								PrintfResp(buf);	//subash commented
...
void send_uart2_data(SIM_MSG_T Uart2Msg)
{
    ...
    if(status != SC_SUCCESS)
    {
        ...
        PrintfResp("send msg error,status = [%d]" );
    }
} */ 
