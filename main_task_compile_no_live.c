...
s_nTimerSettings.m_CycLicOfSec[l_pumpno_tx] */
	...
	//		else if (((Nooftcpprocessed < Nooftcprecvd) || (Nooftcpprocessed1 < Nooftcprecvd1) || livedataflag == 1 || livedataflag1 == 1) && s_nMSettings.m_Enter > 70)
			 if(((Nooftcpprocessed < Nooftcprecvd)||(Nooftcpprocessed1 < Nooftcprecvd1)|| livedataflag==1||livedataflag1==1)&& s_nTimerSettings.m_Enter>=71)  //dg_nsdk
			{
...
status = sAPI_MsgQCreate(&gUart_msgq, "gUart_msgq", (sizeof(SIM_MSG_T)), 20, SC_FIFO);
	if (status != SC_SUCCESS)
	{
	...
	status = sAPI_TaskCreate(&gUartRxProcessTask1,gUartRxProcessTask1Stack, 3* 1024,100,"uart_rx_processer",sTask_UartRxProcesser,(void *)0);
if (SC_SUCCESS != status)
	{
		sAPI_UartPrintf("Task create fail");
	}
